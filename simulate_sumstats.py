...
def main():
    ...
    for i in xrange(args.num_sim):
        ...
        Z1, Z2 = sim_sumstats(args.bfile, annot_names, annot_mat, coef_mat,
                    cont_annot, args.pct_causal, args.sim_hsq,  persnp_hsq1,
                    persnp_hsq2, persnp_cov, out_info, i+1, start_chrom,
                    end_chrom, args.standardize, args.nbin, args.out,
                    args.info_only)
...
def sim_sumstats(bfile, annot_names, annot_mat, coef_mat, cont_annot,
    pct_causal, hsq, persnp_hsq1, persnp_hsq2, persnp_cov, out_info, idx,
    start_chrom, end_chrom, standardize, nbin, out_nm, info_only):
    ...
    ncau = int(np.floor(np.float(legend.shape[0])*pct_causal))
    cau_idx = np.sort(np.random.choice(all_idx, ncau, replace=False))
    ...
    beta1, beta2 = sim_beta(persnp_hsq1, persnp_hsq2, persnp_cov)

    # set non causal to zero
    mask = np.zeros(beta1.shape[0], dtype=bool)
    mask[cau_idx] = True
    beta1[~mask] = np.float32(0.0)
    beta2[~mask] = np.float32(0.0)
    ...
    pheno1 = sim_pheno(bfile1, start_chrom, end_chrom, cau_idx, beta1,
        legend, standardize)
    zsc_maf_thres = 0.01
    ...
    Z1 = sim_zsc(bfile1, start_chrom, end_chrom, pheno1,
    ...
    pheno2 = sim_pheno(bfile2, start_chrom, end_chrom, cau_idx, beta2,
        legend, standardize)
    
    logging.info('Phenotype for population 2 simulated')
   
    out2_idx = np.where(freq2['MAF']>zsc_maf_thres)[0]
    Z2 = sim_zsc(bfile2, start_chrom, end_chrom, pheno2,
...
def sim_pheno(bfile, start_chrom, end_chrom, cau_idx, beta, legend,
...
def sim_zsc(bfile, start_chrom, end_chrom, pheno, legend,
    ...
    for i in xrange(start_chrom, end_chrom+1):
        ...
        snpdata = Bed('{}{}.bed'.format(bfile, i), count_A1=False)
        nindv = np.float32(snpdata.iid_count)
        nsnp = snpdata.sid_count
        blocks = create_block(0, nsnp-1, nblock)
...
def sim_beta(persnp_hsq1, persnp_hsq2, persnp_cov):
